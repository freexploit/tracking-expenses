...
class (Monad m, MonadIO m) => GraphQLMonad m where
    ...
    upsertBank :: Sc.Bank -> m (ResponseStream UpsertBank)
...
instance GraphQLMonad AppM where
    ...
    insertExpenses expenses' = do
        ...
        request client InsertExpensesArgs { Schema.Mutation._data = ss }
    ...
    upsertBank bank' = do
        ...
        ss <- mapM fromExpense  expenses'
        request client Insert { Schema.Mutation._data = ss }
